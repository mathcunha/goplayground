...
func main() {
	...
	for i, mode := range modes {
		...
		for _, slo := range slos {
			...
			fmt.Printf("\"%s\",%.0f,\"%s\",%d\n", labels[i], slo/10000, "convergence", c)
			fmt.Printf("\"%s\",%.0f,\"%s\",%d\n", labels[i], slo/10000, "abscence", d)
			fmt.Printf("\"%s\",%.0f,\"%s\",%d\n", labels[i], slo/10000, "divergence", a)
			fmt.Printf("\"%s\",%.0f,\"%s\",%.4f\n", labels[i], slo/10000, "metric", capacitor.CalcCapacityAccuracy(c, d, a))
